...
class WebsiteEventQuestionController(WebsiteEventSaleController):
    ...
    def registration_new(self, event, **post):
        ...
        if not tickets:
            return request.redirect("/event/%s" % slugify(event))
        return request.env['ir.ui.view'].render_template("website_event_questions_adv.nox_registration_attendee_details", {'tickets': tickets, 'event': event})
